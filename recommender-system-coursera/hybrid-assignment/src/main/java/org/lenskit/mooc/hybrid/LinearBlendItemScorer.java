...
public class LinearBlendItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items) {
        ...
        for (Long item: items) {
            Result suiLeft = leftScorer.score(user, item);
            Result suiRight = rightScorer.score(user, item);
            double bui = biasModel.getIntercept() + biasModel.getUserBias(user) + biasModel.getItemBias(item);
            Result r;
            if (suiLeft == null && suiRight == null) {
                r = Results.create(item, bui);
            } else if (suiLeft == null) {
                double sui = bui + blendWeight * (suiRight.getScore()-bui);
                r = Results.create(item, sui);
            } else if (suiRight == null) {
                double sui = bui + (1-blendWeight) * (suiLeft.getScore()-bui);
                r = Results.create(item, sui);
            } else {
                double sui = bui + (1-blendWeight) * (suiLeft.getScore()-bui)+ blendWeight * (suiRight.getScore()-bui);
                r = Results.create(item, sui);
            }
            results.add(r);
        }
