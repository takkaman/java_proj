...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        logger.info("printing weights {}", weights);
        if (featureCount > 0) {
...
//            logger.info("printing dimensions {}, {}", userMatrix. itemMatrix);
            ...
            logger.info("printing itemMatrix {}", itemMatrix);
    ...
    private RealMatrix createRatingMatrix(KeyIndex userIndex, KeyIndex itemIndex) {
        ...
        try (ObjectStream<Rating> ratings = dao.query(Rating.class)
            ...
            for (Rating r: ratings) {
                //logger.info("printing rating {}, {}, {}", r.getUserId(), r.getItemId(), r.getValue());
                //matrix.addToEntry((int) r.getUserId(), (int) r.getItemId(), r.getValue());
                double bui = r.getValue() - baseline.getIntercept() - baseline.getItemBias(r.getItemId()) - baseline.getUserBias(r.getUserId());
                matrix.setEntry(userIndex.getIndex(r.getUserId()), itemIndex.getIndex(r.getItemId()), bui);
            }
        }
//        logger.info("printing matrix {}", matrix.getEntry(5,5));
