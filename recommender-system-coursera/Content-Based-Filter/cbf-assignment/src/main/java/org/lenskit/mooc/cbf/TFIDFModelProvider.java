...
//                    logger.info("Found tag {} again {}", tag, work.get(tag));
                } else {
                    work.put(tag, Double.valueOf(1));
                }

                //logger.info("Scanning tag {}", tag);
                // TODO Also count it in the document frequency vector when needed
                if (!tmp.containsKey(tag)) {
                    tmp.put(tag, Double.valueOf(1));
                    if (docFreq.containsKey(tag)) {
                        docFreq.put(tag, docFreq.get(tag) + 1);
                    } else {
                        docFreq.put(tag, Double.valueOf(1));
                    }
                }
            }
//            logger.info("scanning {}", work);
            itemVectors.put(item, work);
            //logger.info("Computed TF vectors {}", work);
        }
//        logger.info("Computed DF vectors {}", docFreq.values());
...
//            logger.info("Computed IDF {}", logN - Math.log(e.getValue()));
        }
...
//                logger.info("Computed TFIDF vectors for TF {} IDF {}", e.getValue(), docFreq.get(e.getKey()));
                e.setValue(val);
                length += val * val;
            }
            ...
            for (Map.Entry<String, Double> e: tv.entrySet()) {
                e.setValue(e.getValue()/Math.sqrt(length));
            }
//            logger.info("Computed tv for {}, {}", entry.getKey(), tv);
