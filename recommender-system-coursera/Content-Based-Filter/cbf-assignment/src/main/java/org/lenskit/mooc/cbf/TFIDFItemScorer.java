...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items){
        ...
        Double length_p = Double.valueOf(0);
        ...
        for (Long item: items) {
            ...
            double length_q = Double.valueOf(0);
            double val = Double.valueOf(0);
            for (Map.Entry<String, Double> e: iv.entrySet()) {
                length_q += e.getValue() * e.getValue();
                if (userVector.containsKey(e.getKey())) {
                    val += e.getValue() * userVector.get(e.getKey());
                }
            }
            if (val != 0) {
                Result r = Results.create(item, val / (Math.sqrt(length_p) * Math.sqrt(length_q)));
                results.add(r);
            }
