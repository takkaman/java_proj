...
// loop over the 'y' items
            for (Long2ObjectMap.Entry<LongSortedSet> yEntry: itemUsers.long2ObjectEntrySet()) {
                ...
                Iterator it = yUsers.iterator();
                int count = 0;
                while (it.hasNext()) {
                    if (xUsers.contains((Long)it.next())) {
                        count++;
                    }
                }
                double score = Double.valueOf(count)*Double.valueOf(allUsers.size())/Double.valueOf(xUsers.size()*yUsers.size());
                //logger.info("counting item {}", score);
                itemScores.put(Long.valueOf(yId), Double.valueOf(score));
            }
